...
{
    ofstream outfile;
    outfile.open(user::_id,ofstream::app);
    if(outfile.fail())
    {
        cerr<<"错误：打开文件失败。"<<endl;
        assert(0);
    ...
    */
    ...
    user::mytask.insert(make_pair(tmp_key,tmp_task));
    //printMap(user::mytask);
...
void user::delete_task()//任务删除
{
    ...
    in.close();//关闭流
    out.close();
    fstream outfile(user::_id, ios::out);
...
{
    fstream in(user::_id, ios::in);//原文件
